...
interface ExistingApplication {
  ...
  job_application_experiences?: JobApplicationExperience[];
}
...
export function ApplyJobForm({ jobId, job, countries, existingApplication, applicantId }: ApplyJobFormProps) {
  ...
  useEffect(() => {
    if (existingApplication) {
      ...
      reset({
        ...
        country_id: existingApplication.country_id,
        phone_number: existingApplication.phone_number,
        email: existingApplication.email,
        salary_expectation: existingApplication.salary_expectation,
        additional_expectations: existingApplication.additional_expectations || "",
        experiences: existingApplication.job_application_experiences && existingApplication.job_application_experiences.length > 0 
          ? existingApplication.job_application_experiences.map((exp: JobApplicationExperience) => ({
    ...
    try {
      if (existingApplication) {
        ...
        for (let i = 0; i < data.experiences.length; i++) {
          ...
          const existingExp = existingApplication.job_application_experiences?.[i];
        ...
        if (existingApplication.job_application_experiences) {
          for (const exp of existingApplication.job_application_experiences) {
